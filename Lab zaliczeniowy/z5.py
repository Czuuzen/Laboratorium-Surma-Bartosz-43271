...
def AZN(x):
    y = round(x * 0.59, 2)
    return print(x, "AZN to", y, "USD")

def UAZN(x):
    y = round(x * 1.7, 2)
    return print(x, "USD to", y, "AZN")

def MWK(x):
    y = round(x * 0.0013, 2)
    return print(x, "MWK to", y, "USD")
...
print("6) Manat azerbejdżański (AZN) – Dolary amerykańskie (USD)")
print("7) Kwacha malawijska (MWK) – Dolary amerykańskie (USD)")

waluta = input("Wybierz walutę: ")
if waluta in ('1', '2', '3', '4', '5', '6', '7'):
    kwota = 0
    if waluta == '1':
        ...
        przelicz("AZN", "USD", AZN, UAZN)
    elif waluta == '7':
        przelicz("MWK", "USD", MWK, UMWK)
else:
    print("Wybrałeś niepoprawną opcję")